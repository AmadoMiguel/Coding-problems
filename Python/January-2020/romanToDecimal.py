...
romanNumsMap = {"I": 1, "IV": 4, "V": 5, "X": 10, "XL": 40, "L": 50, "XC": 90,
                "C": 100, "CD": 400, "D": 500, "CM": 900, "M": 1000}


# Roman number input is assumed to be conformed correctly
def convertRomanToDecimal(romanNum):
    decResult = 0
    while len(romanNum):
        print(romanNum)
        # Check if first 2 letters conform a valid roman number. Otherwise, only the first letter
        if len(romanNum) > 1:
            if romanNum[:2] in romanNumsMap.keys():
                decResult += romanNumsMap[romanNum[:2]]
                romanNum = romanNum[2:]
                continue
        decResult += romanNumsMap[romanNum[0]]
        romanNum = romanNum[1:]
    return decResult


print(convertRomanToDecimal("CDIVII"))
